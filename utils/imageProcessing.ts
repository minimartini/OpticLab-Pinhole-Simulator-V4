...
export const applySensorNoise = (data: ImageData, iso: number) => {
    ...
    // Shot noise scales with signal (sqrt(signal)), approximated here by ISO * intensity
    // Read noise is constant-ish
    const isoFactor = Math.max(0, iso - 100);
    const shotNoiseScale = (isoFactor / 6400) * 20; 
    const readNoise = (isoFactor / 3200) * 2;

    for (let i = 0; i < pixels.length; i += 4) {
        // Skip fully transparent pixels
        if (pixels[i+3] === 0) continue;

        const noiseR = (Math.random() - 0.5);
        const noiseG = (Math.random() - 0.5);
        const noiseB = (Math.random() - 0.5);
        
        // Add Noise
        pixels[i] = Math.min(255, Math.max(0, pixels[i] + noiseR * (readNoise + Math.sqrt(pixels[i]) * shotNoiseScale)));
        pixels[i+1] = Math.min(255, Math.max(0, pixels[i+1] + noiseG * (readNoise + Math.sqrt(pixels[i+1]) * shotNoiseScale)));
        pixels[i+2] = Math.min(255, Math.max(0, pixels[i+2] + noiseB * (readNoise + Math.sqrt(pixels[i+2]) * shotNoiseScale)));
    }
};
